...
function ENT:Use(ply)
    ...
    if not ply:GetNWBool("IsScavenging") then
        ...
        //ply:EmitSound(table.Random(scavengingsounds))
        ...
        timer.Simple(10, function()
            ...
            local scrap = ents.Create("bmrp_metal")
            if IsValid(ply) then
                ply:SetNWBool("IsScavenging", false)
                scrap:SetPos(ply:GetPos()+ply:OBBCenter()+Vector(0,0,50))
            else
                scrap:SetPos(self:GetPos()+Vector(0,0,50))
            end
            scrap:Spawn()
            scrap:Activate()
            -- SET CPPI OWNER
            scrap:CPPISetOwner(ply)
