...
util.AddNetworkString("KeypadFunny")


net.Receive("KeypadFunny", function(_, ply)
	local ent = net.ReadEntity()

	if not IsValid(ply) or not IsValid(ent) or ent:GetClass():lower() ~= "funnypad" then
		return
	end
...
function ENT:Process(granted)
	...
	if(granted) then
		...
		self:SetPassword("0000")
		hook.Run("FunnyPadGranted", self)
	else
...
function ENT:SetData(data)
	...
	self:SetPassword(data.Password or "0000")
	self:Reset()
	duplicator.StoreEntityModifier(self, "keypad_funny_password_passthrough", self.KeypadData)
end
...
duplicator.RegisterEntityModifier( "keypad_funny_password_passthrough", function(ply, entity, data)
