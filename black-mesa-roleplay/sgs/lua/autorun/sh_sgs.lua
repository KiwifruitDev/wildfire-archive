// Modified version of https://steamcommunity.com/sharedfiles/filedetails/?id=1342030824 designed to work with websockets.
if SERVER then
	local sg = {}
	local NWStrings = {
		"SGSRequest",
	...
	net.Receive( "SGSRequest", function( _, ply )
		local ent = net.ReadEntity()
		local qual = net.ReadUInt( 32 )
		for k, v in next, player.GetAll() do
			if v.isgrabbing and v ~= ply then
				ply:rtxappend( sg, red, "Error: " .. v:Name() .. "is sging " .. v.sg .. ". To reduce server stress, only one sg can be performed at once." )
				return
			end
		end
		if ply.isgrabbing then
			ply:rtxappend( sg.red, "Error: You are already sging someone" )
			return
		end
	...
	net.Receive( "SGSInitCallback", function( _, ply )
		local usingwebsocket = net.ReadBool()
		local tosend
		if not usingwebsocket then
			tosend = net.ReadEntity()
		end
		...
		ply.parts = parts
		ply.IsSending = true
		if IsValid(tosend) then
			net.Start( "SGSConfirmation" )
	...
	net.Receive( "SGSSendPart", function( _, ply )
		local sendto = ply.sg
		local len = net.ReadUInt( 32 )
		...
		if not ply.data then
			ply.data = {}
			ply.data[ 1 ] = data
			if IsValid(sendto) then sendto:rtxappend( sg.blue, "Received 1st part" ) end
			//print( "1st part received" )
		else
			local num = table.getn( ply.data ) + 1
			ply.data[ num ] = data
			if IsValid(sendto) then sendto:rtxappend( sg.blue, "Received " .. num .. STNDRD( num ) .. " part" ) end
			//print( num .. STNDRD( num ) .. " part received" )
		end
		if table.getn( ply.data ) == ply.parts then
			ply.IsSending = nil
			//print( "Received all parts" )
			hook.Run( "SGSFinishedWebSocket", ply, ply.data, sendto)
	...
	net.Receive( "SGSFinished", function( _, ply )
		local _ply = ply.sg
		_ply.parts = nil
		_ply.data = nil
		ply.parts = nil
	...
	hook.Add( "PlayerDisconnected", "SGSInterrupt", function( ply )
		if ply.IsSending then
			local _ply = ply.sg or {}
			_ply.parts = nil
			_ply.data = nil
			ply.parts = nil
			ply.data = nil
...
if CLIENT then
    ...
    local shouldSGS = false
	local usingwebsocket = false
    local sgQuality
    local sgPly
	local function cl_rtxappend2( color, text, ply )
		//print(text)
		if usingwebsocket then return end -- A player did not initialize this
    ...
    net.Receive( "StartSGS", function()
        shouldSGS = true
		usingwebsocket = false
        sgQuality = net.ReadUInt(32)
        sgPly = net.ReadEntity()
    end)
	function StartSGS_WebSocket(quality)
        shouldSGS = true
		usingwebsocket = true
        sgQuality = quality
    end
    hook.Add("PostRender", "SGS", function()
        if (!shouldSGS) then return end
        shouldSGS = false
        ...
        local function capture( q )
            ...
            net.Start( "SGSInitCallback" )
				net.WriteBool( usingwebsocket )
				if not usingwebsocket then
            ...
            local i = 1
            timer.Create( "SGSSendParts", 0.1, amt, function()
                net.Start( "SGSSendPart" )
	...
	local function DisplayData( str, name )
		...
		if not name then
			main:SetTitle( "SGS of " .. LocalPlayer().gfname .. " (" .. string.len( str ) .. " bytes, took " .. elapsedtime .. " seconds)" )
		else
	...
	net.Receive( "SGSInterrupted", function()
	...
	net.Receive( "SGSConfirmation", function()
		local parts = net.ReadUInt( 32 )
		local len = net.ReadUInt( 32 )
		local time = net.ReadFloat()
		local ent = net.ReadEntity()
	...
	function OpenSGMenu()
		...
		main.Paint = function()
			...
			surface.SetTextPos( main:GetWide() / 2 - surface.GetTextSize( "SGS Menu" ) / 2, 5 ) 
			surface.SetTextColor( 255, 255, 255, 255 )
			surface.DrawText( "SGS Menu" )
		end
