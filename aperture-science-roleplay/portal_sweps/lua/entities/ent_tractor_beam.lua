...
function ENT:Enable(enable)
	if self:GetEnable() != enable then
		if enable then
			...
			self.tbeam_loop = CreateSound (self,"TA:TractorBeamInside")
	        self.tbeam_loop:Play()
...
function ENT:OnEnterFunnel(ent)
	...
	if ent:IsPlayer() then
		...
		ent:EmitSound("TA:TractorBeamInside")
...
function ENT:OnLeaveFunnel(ent)
	...
	if ent:IsPlayer() then
		...
		ent:StopSound("TA:TractorBeamInside")
