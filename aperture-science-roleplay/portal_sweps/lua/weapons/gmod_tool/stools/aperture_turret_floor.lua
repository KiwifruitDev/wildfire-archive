...
if CLIENT then
	language.Add("tool.aperture_turret_floor.name", "Turret")
	language.Add("tool.aperture_turret_floor.desc", "Shoots at all living things.")
	language.Add("tool.aperture_turret_floor.0", "LMB: Place")
	language.Add("tool.aperture_turret_floor.enable", "Enable")
	language.Add("tool.aperture_turret_floor.startenabled", "Starts active")
	language.Add("tool.aperture_turret_floor.startenabled.help", "Turret will start active after being placed.")
...
function TOOL:LeftClick(trace)
	...
	if math.Round(normal.z) != 1 and model != "models/aperture/rocket_sentry.mdl" then
		PrintMessage(HUD_PRINTCENTER, "Туррель можно разместить только на пол.")
		return
	end
