...
if CLIENT then
	language.Add("tool.aperture_laser_catcher.name", "Laser receiver")
	language.Add("tool.aperture_laser_catcher.desc", "Gives out a signal when a laser beam hits it.")
	language.Add("tool.aperture_laser_catcher.enable", "Key to activate (unused)")
	language.Add("tool.aperture_laser_catcher.timer", "Time before release (unused)")
	language.Add("tool.aperture_laser_catcher.0", "LMB: Place")
end
...
function TOOL:UpdateGhostLaserEmitter(ent, ply)
	...
	local model = self:GetClientInfo("model")
	local offsets = self:ModelToOffsets(model)
	if not offsets then offsets = {z = 0, ang = Angle()} end
	local pos = trace.HitPos + trace.HitNormal * offsets.z
	local ang = trace.HitNormal:Angle() + offsets.ang
