...
if ( SERVER ) then
		...
		ballSpeed = CreateConVar("portal_projectile_speed", 10000, {FCVAR_NOTIFY,FCVAR_ARCHIVE,FCVAR_REPLICATED,FCVAR_SERVER_CAN_EXECUTE}, "The speed that portal projectiles travel.")
...
SWEP.MelModel 					= "models/weapons/melgun/v_portalgun.mdl"
SWEP.MelWorldModel 				= "models/weapons/melgun/w_portalgun.mdl"
...
--SWEP.ShootOrange        = Sound( "weapons/portalgun/wpn_portal_gun_fire_red1.wav")
--SWEP.ShootBlue        = Sound( "weapons/portalgun/wpn_portal_gun_fire_blue2.wav")
...
function SWEP:Equip( NewOwner )
	...
	local portalFlipper = GetGlobalInt("PortalFlipper")
	if SERVER then
		if portalFlipper <= 0 or portalFlipper >= 20 then
			SetGlobalInt("PortalFlipper", -1)
		end
		SetGlobalInt("PortalFlipper", portalFlipper + 2)
		self:SetNWInt("portalFlipper", portalFlipper)
	end
	print("Equipped " .. portalFlipper + 2)
	if math.random(2,2) == 1 then
		self.IsMelPortalGun = true
		self.ViewModel = self.MelModel
		self.WorldModel = self.MelWorldModel
	else
		self.IsMelPortal = false
	end
	self.IsSetup = true
end
	...
	if IsFirstTimePredicted() and owner:IsValid() then --Predict that motha' fucka'
		...
		if SERVER then
			...
			if ( trace.Hit and trace.HitWorld ) then
				...
				-- WILDFIRE CHANGE --
				local noportal = trace.MatType == MAT_METAL and Material(trace.HitTexture):GetInt("%noportal") != 1 or trace.MatType != MAT_METAL and true or false
			   	// trace.MatType != MAT_METAL
				if noportal and !trace.HitNoDraw and !trace.HitSky and (trace.MatType != MAT_GLASS or ( trace.MatType == MAT_CONCRETE or trace.MatType == MAT_DIRT ) ) and validpos and validnormang then
					---------------------
					  ...
					  timer.Simple( hitDelay - .05, function()
							if ball and ball:IsValid() then 
								...
								if !IsValid( EntToUse ) then
										...
										print("Flipper")
										Portal:SetColorSlots(portalflipper, LinkedPortalEnums[portalflipper])
										if type == TYPE_BLUE then
												self:SetNWInt("lastPortalFlipper", portalflipper)
												owner:SetNWEntity( "Portal:Blue", Portal )
												Portal:SetNetworkedBool("blue",true,true)
											   
										else
												self:SetNWInt("lastPortalFlipper", LinkedPortalEnums[portalflipper])
						...
						ent:EmitSound( "weapons/portalgun/portal_invalid_surface0.wav")
						ent:EmitSound( "weapons/portalgun/portal_invalid_surface"..math.random(1,4)..".wav")
...
function SWEP:SecondaryAttack()
		...
		self.Weapon:EmitSound( "weapons/portalgun/wpn_portal_gun_fire_red"..math.random(1,3)..".wav", 70, 100, .7, CHAN_WEAPON )
...
function SWEP:PrimaryAttack()
		...
		self.Weapon:SendWeaponAnim( ACT_VM_PRIMARYATTACK )
		self.Weapon:EmitSound( "weapons/portalgun/wpn_portal_gun_fire_blue"..math.random(1,3)..".wav", 70, 100, .7, CHAN_WEAPON )
		self.Owner:SetAnimation(PLAYER_ATTACK1)
		self:IdleStuff()
...
function SWEP:CleanPortals()
        ...
        if cleaned then
			...
			self.Weapon:EmitSound( "weapons/portalgun/wpn_portal_fizzler_shimmy_01.wav", 45, 100, .5, CHAN_WEAPON )
			self:IdleStuff()
