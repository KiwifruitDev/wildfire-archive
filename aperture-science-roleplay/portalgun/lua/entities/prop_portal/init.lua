...
sound.Add({
	...
	level = 45, 
	pitch = {95, 110}, 
	sound = "weapons/portalgun/wpn_portal_ambient_lp_01.wav"
})
...
function ENT:CleanMeUp()
	self.portal_loop:Stop()
	self.portal_loop2:Stop()
	self:BootPlayer()
...
local volp = 0 + (2 * (ent:GetVelocity():Length() - 100)/1500)

if volp>=1 then volp = 1 end

local volq = 0 + (2 * (ent:GetVelocity():Length() - 100)/3000)

if volq>=0.5 then volq = 0.5 end
	...
	channel = 		CHAN_AUTO,
	volume = 		volp,
    pitch = 80 + (25 * (ent:GetVelocity():Length() - 100)/500),
	sound = 			"weapons/portalgun/portal_enter"..math.random(1,3)..".wav"
})
	...
	channel = 		CHAN_AUTO,
	volume = 		volq,
    pitch = 90 + (25 * (ent:GetVelocity():Length() - 100)/500),
	sound = 			"weapons/portalgun/portal_exit"..math.random(1,2)..".wav" 
})

	if ent.JustEntered then
		ent:EmitSound( "portal.in")
